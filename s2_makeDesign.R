...
outpath <- '/space/syn50/1/data/ABCD/d9smith/age/designMat'
...
contvar <- c(paste0('bf_demean_',c(1,2,4,5)), 'PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8','PC9', 'PC10', 'dmri_meanmotion', 'household.income_cont')
...
# Continuous variables
contvar <- c(paste0('bf_demean_',c(1,2,4,5)), 'pds_y_p_average', 'PC1', 'PC2', 'PC3', 'PC4', 'PC5', 'PC6', 'PC7', 'PC8','PC9', 'PC10', 'dmri_meanmotion', 'household.income_cont')
...
############################################################
# Design Matrix 5_red: same as #5 but remove the column with PDS
full = read.delim(outfile)
reduced = subset(full, select = -c(pds_y_p_average))
fname <- 'designMat5_red_BFsSexIncEducHispPCsScanSoftMotion_bly2y4.txt'
outfile <- paste0(outpath, '/', fname)
write.table(reduced, outfile, sep = '\t', row.names = FALSE)
...
############################################################
# Design Matrix 5f_red: same as #5f but remove the column with PDS
full = read.delim(outfile)
reduced = subset(full, select = -c(pds_y_p_average))
fname <- 'designMat5f_red_BFsSexIncEducHispPCsScanSoftMotion_bly2y4.txt'
outfile <- paste0(outpath, '/', fname)
write.table(reduced, outfile, sep = '\t', row.names = FALSE)
...
############################################################
# Design Matrix 5m_red: same as #5f but remove the column with PDS
full = read.delim(outfile)
reduced = subset(full, select = -c(pds_y_p_average))
fname <- 'designMat5m_red_BFsSexIncEducHispPCsScanSoftMotion_bly2y4.txt'
outfile <- paste0(outpath, '/', fname)
write.table(reduced, outfile, sep = '\t', row.names = FALSE)
